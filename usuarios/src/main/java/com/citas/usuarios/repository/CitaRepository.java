...
public interface CitaRepository extends JpaRepository<Citas, Long> {
        ...
        List<Citas> findByIdHorario(Horario_disponible idHorario);

        List<Citas> findByEstado(Integer estadoCita);
         ...
         */
        @Query("SELECT c FROM Citas c WHERE c.empleado.usuario.id = :idUsuario")
        List<Citas> buscarCitasPorUsuarioEmpleado(@Param("idUsuario") Integer idUsuario);
